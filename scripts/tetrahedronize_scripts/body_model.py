...
class SMPLModel():
    def __init__(self, model_path, age):
        ...
        if age == 'kid':
            v_template_smil = np.load(os.path.join(
                os.path.dirname(model_path), "smpl_kid_template.npy"))
            v_template_smil -= np.mean(v_template_smil, axis=0)
            v_template_diff = np.expand_dims(
                v_template_smil - self.v_template, axis=2)
    ...
    def update(self):
        ...
        v = np.matmul(T, rest_shape_h.reshape(
            [-1, 4, 1])).reshape([-1, 4])[:, :3]
...
class TetraSMPLModel():
    ...
    def update(self):
        ...
        v_added = np.matmul(T_added, rest_shape_added_h.reshape(
            [-1, 4, 1])).reshape([-1, 4])[:, :3]
