...
if not pcd:
    vertices, faces, normals, faces_normals, textures, face_textures = load_scan(mesh_file, with_normal=True, with_texture=True)
else:
...
if pcd:
    ...
    # fake multiseg
    vertices_label_mode = np.random.randint(low=1, high=10, size=(vertices.shape[0], 10))   # [scan_verts_n, percomp]
    colormap = cm.get_cmap("rainbow")
    precomp_id = 4
    verts_label = colormap(vertices_label_mode[:, precomp_id]/np.max(vertices_label_mode[:, precomp_id]))[:,:3] #[scan_verts_num, 3]
