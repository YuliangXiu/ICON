...
class ICON(pl.LightningModule):
    ...
    def optim_body(self, in_tensor_dict, batch):
        ...
        for i in loop_smpl:
            ...
            smpl_out = smpl_model(betas=optimed_betas,
            ...
            self.render.load_simple_mesh(smpl_verts,
                                         in_tensor_dict['smpl_faces'])
            in_tensor_dict['T_normal_F'], in_tensor_dict[
                'T_normal_B'] = self.render.get_clean_image()
            T_mask_F, T_mask_B = self.render.get_silhouette_image()

            with torch.no_grad():
                in_tensor_dict['normal_F'], in_tensor_dict[
                    'normal_B'] = self.netG.normal_filter(in_tensor_dict)
        ...
        in_tensor_dict.update(
            self.compute_vis_cmap(batch['type'][0],
    ...
    def optim_cloth(self, verts_pr, faces_pr, inter):
        ...
        for i in loop_cloth:
            ...
            self.render.load_simple_mesh(
                ...
                faces_pr.unsqueeze(0).to(self.device).long(), deform_verts)
    ...
    def test_step(self, batch, batch_idx):
        ...
        in_tensor_dict.update(
            self.evaluator.render_normal(batch['smpl_verts'],
                                         batch['smpl_faces']))
        ...
        # save inter results
        image = (in_tensor_dict['image'][0].permute(
            1, 2, 0).detach().cpu().numpy() + 1.0) * 0.5
        smpl_F = (in_tensor_dict['T_normal_F'][0].permute(
            1, 2, 0).detach().cpu().numpy() + 1.0) * 0.5
        smpl_B = (in_tensor_dict['T_normal_B'][0].permute(
            1, 2, 0).detach().cpu().numpy() + 1.0) * 0.5
    ...
    def test_epoch_end(self, outputs):
        ...
        accu_outputs = accumulate(outputs,
                                  rot_num=3,
                                  split={
                                      'renderpeople': (0, 50),
                                      'cape-easy': (50, 100),
        ...
        self.logger.experiment.add_hparams(hparam_dict={
            'lr_G': self.lr_G,
            'bsize': self.batch_size
        },
                                           metric_dict=accu_outputs)

        np.save(osp.join(self.export_dir, "../test_results.npy"),
                accu_outputs,
                allow_pickle=True)
    ...
    def render_func(self, in_tensor_dict, dataset='title', idx=0):
        ...
        if sdf is not None:
            ...
            image_gt = resize(
                ((in_tensor_dict['image'].cpu().numpy()[0] + 1.0) /
                 2.0).transpose(1, 2, 0), (height, height),
                anti_aliasing=True)
