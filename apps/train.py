...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-test', '--test_mode', action='store_true')
    ...
    if cfg.test_mode or args.test_mode:
        ...
        cfg_test_mode = ['test_mode', True,
                         'dataset.types', ["renderpeople", "cape"],
                         'dataset.rp_type', "pifu450",
                         'dataset.scales', [1.0, 100.0]]
    ...
    if not cfg.test_mode:
        ...
        if cfg.overfit:
            cfg_show_list = ['freq_show_train', 100.0,
                             'freq_show_val', 10.0]
        else:
    ...
    if cfg.resume and os.path.exists(cfg.resume_path) and cfg.resume_path.endswith("ckpt"):
        ...
        trainer = SubTrainer(**trainer_kwargs)
        print(colored(f"Resume weights and hparams from {cfg.resume_path}",'green'))
        ...
        if os.path.exists(cfg.resume_path) and cfg.resume_path.endswith("ckpt"):
            ...
            main_dict = {k: v for k, v in main_dict.items()
                                if k in model_dict and v.shape == model_dict[k].shape
                                and ('reconEngine' not in k) and ("normal_filter" not in k) and ('voxelization' not in k)}
        ...
        if os.path.exists(cfg.normal_path) and cfg.normal_path.endswith("ckpt"):
            ...
            normal_dict = {k: v for k, v in normal_dict.items()
                                if k in model_dict and v.shape == model_dict[k].shape}
