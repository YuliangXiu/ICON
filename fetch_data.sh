...
# ICON
echo -e "\nYou need to register at https://icon.is.tue.mpg.de/"
...
function download_for_training () {
    ...
    # SMPL-X (optional)
    echo -e "\n(Optional) You need to register at https://smpl-x.is.tue.mpg.de/"
    ...
    # SMIL (optional)
    echo -e "\n(Optional) You need to register at https://agora.is.tue.mpg.de/"
    read -p "Username (SMIL):" username
    read -p "Password (SMIL):" password
    username=$(urle $username)
    password=$(urle $password)
    wget --post-data "username=$username&password=$password" 'https://download.is.tue.mpg.de/download.php?domain=agora&sfile=smpl_kid_template.npy&resume=1' -O './data/smpl_related/models/smpl/smpl_kid_template.npy' --no-check-certificate --continue
    wget --post-data "username=$username&password=$password" 'https://download.is.tue.mpg.de/download.php?domain=agora&sfile=smplx_kid_template.npy&resume=1' -O './data/smpl_related/models/smplx/smplx_kid_template.npy' --no-check-certificate --continue
}


read -p "(optional) Download models used for training (y/n)?" choice
case "$choice" in 
  y|Y ) download_for_training;;
