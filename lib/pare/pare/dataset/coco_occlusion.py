...
def occlude_with_coco_objects(im,
                              kp2d,
                              occluders,
                              img_size=224,
                              max_n_objects=4):
    ...
    for _ in range(count):
        ...
        try:
            occluder_obj_id = random.choice(
                np.argwhere(occluders['obj_class'] == occluder_info[0]))[0]
        except:
            continue
        ...
        scale_factor = 1. / (occluder_obj_mask.shape[0] /
                             img_size) * np.random.uniform(0.05, 0.7)
